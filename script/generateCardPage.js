...
const generateCardPage = () => {
  ...
  const generateContent = (data) => {
    ...
    const breadcrambList = document.querySelector('.breadcrumb__list');
    const goodsImages = document.querySelector('.good-images');
    const goodsItems = document.querySelector('.good-item');

    breadcrambList.textContent = '';
    goodsImages.textContent = '';
    goodsItems.textContent = '';

    let contentBreadcramb = '';
    let contentImages = '';
    let contentItems = '';

    // добавляем значения свойств из объекта в переменные с помощью деструктуризации
    ...
    contentItems = `
      ...
      <div class="good-item__section">
        <div>
          <h2 class="good-item__header">${itemName}</h2>
      ...
      <div class="good-item__buttons">
        ...
        <button type="button" class="btn btn-add-wishlist" data-idd="${id}">
    ...
    // вставляем вёрстку в нужные блоки
    breadcrambList.insertAdjacentHTML('afterbegin', contentBreadcramb);
    goodsImages.insertAdjacentHTML('afterbegin', contentImages);
    goodsItems.insertAdjacentHTML('afterbegin', contentItems);
    ...
    const checkWishList = () =>  {
      /* в условии вызывается геттер из userData, возвращает список wishListData
      и проверяется, есть ли в этом списке передаваемый id */
    ...
    // при нажатии на кнопку добавления товара в список понравившегося
  ...
  //когда заходим в какой-то товар
  if(location.hash && location.pathname.includes('card')) {
    //console.log(location.hash.substr(1));
    getData.item(location.hash.substr(1), (result) => {
      //console.log(result);
